...
function Profile({
                     error, onUpdateProfile, onLogout, className
                 }) {
    const currentUser = useContext(CurrentUserContext);
    const [formValue, setFormValue] = useState({name: currentUser.name, email: currentUser.email});
    const [isFormEditable, setFormEditable] = useState(true);
    const {handleValidation, errors, isFormValid} = useValidate(FORMS.profile.name, FORMS.profile.requiredField);
        ...
        <section className={className} aria-label={'Профиль'}>
            ...
            <Form className={'profile'} onSubmit={handleSubmit} error={error} isEditable={isFormEditable}
                ...
                <div className={"profile__actions"}>
                    <button
                        className={`profile__edit-btn profile__bnt ${!isFormEditable && 'profile__bnt_hidden'}`}
                        type="button"
